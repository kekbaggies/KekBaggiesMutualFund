...
interface IKekBaggiesMutualFund {
...
contract KekBaggiesMutualFundETHRamp {
    using SafeERC20 for IERC20;

    // Basis points
    uint256 private constant MAX_BP = 10**4;

    // Kek Baggies Mutual Fund contract
    IKekBaggiesMutualFund private immutable kekBaggiesMutualFund;
    // Kek Baggies Mutual Fund reserve token contract
    IERC20 private immutable reserveToken;
    // Kek Baggies Mutual Fund share token contract
    ...
    constructor(address _kekBaggiesMutualFundAddress, address _uniswapV2Router02Address) {
        kekBaggiesMutualFund = IKekBaggiesMutualFund(_kekBaggiesMutualFundAddress);
        reserveToken = IERC20(kekBaggiesMutualFund.getReserveTokenAddress());
        shareToken = IERC20(kekBaggiesMutualFund.getShareTokenAddress());
    ...
    function depositETH() public payable {
        ...
        uint256 minimumDepositReserveToken = kekBaggiesMutualFund.getMinimumDeposit();
        uint256 slippageTolerance = kekBaggiesMutualFund.getSlippageTolerance();
        ...
        reserveToken.approve(
            address(kekBaggiesMutualFund),
            reserveToken.balanceOf(address(this))
        );
        kekBaggiesMutualFund.deposit(reserveTokenBalance);
    ...
    function withdrawETH(uint256 shares, address to) public {
        ...
        kekBaggiesMutualFund.withdraw(shares, address(this));
